...
/*const VDJ = {
	...
	execute(vdjscript) {
		try {
			const r = await fetch(`${this.base}/execute?script=${encodeURIComponent(vdjscript)}`);
			return await r.text();
		}
		catch {
			alert("Failed to execute() in VDJ_API");
	...
	async query(vdjscript) {
		try {
			const r = await fetch(`${this.base}/query?script=${encodeURIComponent(vdjscript)}`, {cache: "no-store"});
			return await r.text();
		}
		catch {
			alert("Failed to query() in VDJ_API");
